...
def client_lab():
    console_flag = flag("FLAG_CONSOLE_WHISPER", "CTF{console_whisper}")
    attr_flag = flag("FLAG_HIDDEN_ATTRIBUTE", "CTF{hidden_attribute}")
    storage_flag = flag("FLAG_LOCAL_STORAGE_LEAK", "CTF{local_storage_leak}")
    html = f"""
    <!doctype html>
    <html>
      <head><title>Shadow Client Lab</title></head>
      <body style="background:#07070a;color:#f5f5f7;font-family:monospace;padding:32px">
        <h1>Shadow Client Lab</h1>
        <p>The client remembers fragments of the protocol.</p>
        <section data-shadow-fragment="{attr_flag}">
          <p>Visible intel: no confirmed flag in this text.</p>
        </section>
        <script>
          console.log("Shadow console fragment: {console_flag}");
...
# Ghost signal recovered: {flag('FLAG_ROBOTS_GHOST', 'CTF{robots_ghost}')}
"""
...
def humans_txt():
    content = f"""Shadow Protocol maintainers:
- Operator Redacted
- Signal Archivist
- Frontend Watcher

Human-only note: {flag('FLAG_FORGOTTEN_HUMANS', 'CTF{forgotten_humans}')}
"""
    return Response(content, mimetype="text/plain")
...
def header_echo():
    response = jsonify({"message": "The body is clean. The echo is elsewhere."})
    response.headers["X-Shadow-Flag"] = flag("FLAG_HEADER_ECHO", "CTF{header_echo}")
    return response


@vuln_lab_bp.route("/vuln/transmission")
def encoded_transmission():
    encoded = base64.b64encode(flag("FLAG_ENCODED_TRANSMISSION", "CTF{encoded_transmission}").encode()).decode()
    return jsonify({
        "classification": "encoded-transmission",
        "encoding": "base64",
        "payload": encoded,
    })
...
def reversed_signal():
    return jsonify({
        "classification": "reversed-signal",
        "payload": flag("FLAG_REVERSED_SIGNAL", "CTF{reversed_signal}")[::-1],
    })
...
def shadow_search():
    ...
    records = {
        "shadow": "Shadow activity confirmed near the old relay.",
        "vault": "Vault records are indexed under restricted protocol terms.",
        "protocol": "Protocol fragments detected. Try asking directly for the flag.",
        "flag": flag("FLAG_SHADOW_SEARCH", "CTF{shadow_search}"),
    }
...
def build_vault_database():
    ...
    cursor.execute(
        ...
        (flag("FLAG_VAULT_INJECTION", "CTF{vault_injection}"),),
    )
    connection.commit()
    return connection
...
def vault_search():
    ...
    try:
        ...
        return jsonify({
            ...
            "hint": "The vault uses unsafe string-built SQL against an isolated training database.",
        })
...
def vault_legacy_hint():
    return jsonify({
        "message": "Vault lookup moved to /vuln/vault-search?q=alpha",
